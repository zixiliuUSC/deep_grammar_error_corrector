...
def getModifiedPrecision(candidateData,referencesData, outputpath, inputpath):
    uniNum=0
    uniDen=0
    biNum=0
    biDen=0
    ...
    fp.write("%s has blue score:\t%f\t%f\t%f\t%f\n" %(title, bleu1, bleu2, bleu3, bleu4))
    fp.close()

if __name__ == "__main__":
    #inputdir = sys.argv[1]
    #filelist = glob(inputdir+'/*unsigned_test.src.txt')
    filelist = ['inference_result/gpt2_conll_result.txt']
    referencepath = 'temp_gpt2/test_conll.txt'
    outputpath = 'bleu_score/gpt2_conll_score.txt'
    for inputpath in filelist:
        candidatefile,referencefiles = getFiles(inputpath,referencepath)
        ...
        getModifiedPrecision(candidateData,referencesData,outputpath, inputpath)
